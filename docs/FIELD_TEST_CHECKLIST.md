...
- **Stray tags in range produce phantom rows.** Observed live (2026-05-13) and initially logged as BUG-002. Root cause: when an unregistered EPC Gen 2 tag is anywhere within RF range of the antenna (a tag stuck to the reader chassis, a forgotten demo tag on the table, one in a bag/pocket within ~1 m), the reader reads it continuously and produces a participant row that looks like a duplicate of the rider's intended tag. Standings will show two rows; whichever tag was waved second may show `laps: 1` and not advance because the W-002 cooldown is in effect for it. Not a code bug — an environment / operator concern. Pre-race procedure: before clicking **Start race**, watch the reader's web-portal `tag.db` page for any tag with a rapidly-incrementing repeat count (>50 reads in a few seconds). Locate and remove (move ≥5 m away or wrap in aluminium foil) before starting. Code mitigation we could add later: a "suppress always-present tags" filter in the reader-service that ignores any tag whose `repeat` field grows monotonically without ever triggering a depart event.
