...
- **Unwanted tags in standings.** Observed live (2026-05-13). A tag the operator is not actively waving — sometimes within RF range despite being in another room — produces a phantom participant row. Cooldown stops it from accumulating laps but the row stays visible. Full diagnosis, diagnostic recipe, and four mitigation options in [`docs/issues/BUG-003-tag-bleed-through.md`](issues/BUG-003-tag-bleed-through.md). For the field test: lower antenna TX power on-site (e.g. `antennas.1.conducted_power=160` via SSH) and bring aluminium foil for unwanted-tag shielding. Software fix (skip non-registered tags in standings) is drafted in the issue file.
